<template>
    <form @submit.prevent="formSubmit" action="" method="POST" class="post-form" enctype="multipart/form-data">
                ...
                <div class="input content-block">
                  <textarea v-model="content" type="text" name="content" id="content" placeholder="Content" maxlength="500" required></textarea>
                ...
                <div class="create-submit">
                  <button type="submit">
                    <span v-show="!submitting">Edit Post</span>
...
export default({
    ...
    methods: {
        ...
        async makeRequestWithJWT(url, options) {
            ...
            if (response.status === 201) {
                ...
                this.$emit('toast', 'Edited successfully')
            }
        ...
        async formSubmit(e) {
            if (this.title.length > 50 || this.content.length > 500)
                return
            ...
            const url = `${this.$store.state.api_url}/post/${this.$route.params.id}`
            const options = {
                method: 'PUT',
                headers: {
    ...
    async beforeMount() {
        this.$store.commit('changePageTitle', 'Edit Post')
        const options = {
            method: 'GET',
            headers: {
            "Authorization": `Bearer ${localStorage.getItem('token')}`
